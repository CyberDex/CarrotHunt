    		...
    		if (enemyHitPlatform) {
    			if (hero.body.y == enemy.body.y) {
    				for (i = 0; i < platforms.length; i++) { // пробегаем по всем существующим платформам
    					...
    					if ((thePlatform.body.y > enemy.position.y - 200) && (thePlatform.body.y < enemy.position.y)) // если платформа не выше 200px от врага
    						if ((thePlatform.body.x < enemy.body.x) && (thePlatform.body.x + thePlatform.body.width > enemy.body.x)) // не прыгаем пока мы под платформой
    							canJump = 1;
    				}
    			}
    		} else {
